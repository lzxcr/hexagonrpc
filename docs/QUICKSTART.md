...
hexagonrpcd 为 DSP 提供虚拟文件系统，默认从 `/usr/share/qcom/` 提供文件。
...
├── dsp/{dsp}/      → /vendor/dsp/{dsp}/
├── sensors/
│   ├── config/     → /vendor/etc/sensors/config/
│   ├── registry/   → /mnt/vendor/persist/sensors/registry/
...
hexagonrpcd 读取 `/proc/device-tree/compatible` 和 `/proc/device-tree/model` 自动猜测设备目录。例如设备 `/usr/share/qcom/sdm845/SHIFT/axolotl` 会被自动检测。
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `Could not open FastRPC node` | 设备节点不存在或权限不足 | 检查 `/dev/fastrpc-*`，确保用户在 `hexagonrpc` 组 |
...
项目提供 5 个 systemd service 单元（`data/` 目录），安装后可通过 udev 自动启动：
